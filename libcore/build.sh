...
ls -la
rm -f ${{ github.workspace }}/tbax/sing-box/outbound/shadowsocksr.go
rm -f ${{ github.workspace }}/tbax/sing-box/outbound/shadowsocksr_stub.go
mv -f spa/.github/outbound/* ${{ github.workspace }}/tbax/sing-box/outbound/
mv -f spa/.github/test/* ${{ github.workspace }}/tbax/sing-box/test/
mv -f spa/.github/transport/* ${{ github.workspace }}/tbax/sing-box/transport/
sed -i '0,/berty.tech\/go-libtor v[0-999]\+\.[0-999]\+\.[0-999]\+/s//&\n\tgithub.com\/Dreamacro\/clash v1.17.0/' ${{ github.workspace }}/tbax/sing-box/go.mod
awk '/github.com\/ajg\/form v[0-9]+\.[0-9]+\.[0-9]+ \/\/ indirect/ && !inserted {print "\tgithub.com/Dreamacro/protobytes v0.0.0-20230617041236-6500a9f4f158 // indirect"; inserted=1} 1' ${{ github.workspace }}/tbax/sing-box/go.mod
sed -i -e '/ImageShadowTLS             = "ghcr.io\/ihciah\/shadow-tls:latest"/ { N; s/ImageShadowTLS             = "ghcr.io\/ihciah\/shadow-tls:latest"\n/ImageShadowTLS             = "ghcr.io\/ihciah\/shadow-tls:latest"\n\tImageShadowsocksR          = "teddysun\/shadowsocks-r:latest"\n/ }' ${{ github.workspace }}/tbax/sing-box/test/clash_test.go
